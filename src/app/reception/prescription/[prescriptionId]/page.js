...
function PrescriptionDetail() {
  ...
  const doctorIndex = path.indexOf("reception/prescription/");
  const prescriptionId =
    doctorIndex !== -1
      ? path.slice(doctorIndex + "reception/prescription/".length)
      : "";
      ...
      try {
        ...
        if (prescriptionId) {
          ...
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/reception/prescriptionn/${prescriptionId}`,
            {
              headers: {
